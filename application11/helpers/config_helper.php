...
if (!function_exists('getyear')) {
    function getyear($syr = 0) {
        // Default start year is 2010 if none provided
        $syear = $syr == 0 ? 2020 : $syr;

        // Generate array of years from $syear to (current year + 1)
        $currentYear = date('Y') + 40;
        $arrYears = [];

        foreach (range($syear, $currentYear) as $yr) {
            $arrYears[] = $yr;
        }
...
if ( ! function_exists('constWorkHrs'))
{
    ...
    {
    	$hours = (strtotime('08:00') - strtotime($break)) / 3600;
...
if ( ! function_exists('salaryPeriod'))
{
    ...
    {
        return array('Monthly','Semi-monthly','Weekly','Daily');
    }
}
