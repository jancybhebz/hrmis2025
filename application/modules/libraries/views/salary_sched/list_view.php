                                ...
                                <?php foreach($stepNumber as $column): ?>
                                    <td><?php
                                           ...
                                           // echo '<a href="'.base_url('libraries/salary_sched/edit/'.$row['salaryGradeNumber'].'/'.$column['stepNumber'].'/'.$actual_salary.'/'.$intVersion).'">'.$actual_salary.'</a>';
                                            if (count($keys) > 0) {
										    $salary_raw = $arrSalarysched[$keys[0]]['actualSalary'];   // raw value for URL
    $actual_salary = number_format($salary_raw, 2);            // formatted for display
} else {
    $salary_raw = '';
    $actual_salary = '';
}

echo '<a href="'.base_url('libraries/salary_sched/edit/'
    .$row['salaryGradeNumber'].'/'
    .$column['stepNumber'].'/'
    .$salary_raw.'/'   // 👈 raw, no comma
    .$intVersion).'">'.$actual_salary.'</a>';
                                        ?></td>
                                <?php endforeach; ?>
                            </tr>
                        <?php endforeach; ?>
